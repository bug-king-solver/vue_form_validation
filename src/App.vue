<template>
  <div id="app">
    <form action="" @submit.prevent="submit($event)">
      <p v-for="(item, index) in $v.elems.$each.$iter" :key="index">
        <input
          type="text"
          :class="{
            /* you can replace this with a function if you like */
            'form-group--error':
              item.$error /* these is the state class for error */,
            'form-group--success': !item.$error && item.$dirty,
            /* these is the state class for success */
          }"
          v-model="item.$model.value"
          @blur="item.$model.value !== '' && item.$touch()"
        />
        <button type="button" @click="remove(index)">delete</button>
      </p>
      <br />
      <div>
        <button type="button" class="btn-primary edit" @click="add">Add</button>
        &nbsp;
